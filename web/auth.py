#!/usr/bin/env python3
"""Proxy Manager Web 登录认证。

凭据从项目 data/web.env 读取（WEB_USER / WEB_PASS），不写死在代码里，
避免随仓库泄露。若文件不存在，回退到 admin/admin123 并打警告（仅兜底，
正式部署请由 webpanel.sh 生成随机密码的 web.env）。

会话用随机令牌（非明文 "ok"），存于进程内集合，cookie 仅持有令牌。
"""
import os
import secrets

BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
WEB_ENV = os.path.join(BASE_DIR, "data", "web.env")

# 进程内会话令牌集合（单进程 uvicorn 足够；多 worker 需换外部存储）
SESSIONS = set()

_FALLBACK_USER = "admin"
_FALLBACK_PASS = "admin123"


def load_creds():
    user, pw = _FALLBACK_USER, _FALLBACK_PASS
    if os.path.exists(WEB_ENV):
        with open(WEB_ENV) as f:
            for line in f:
                line = line.strip()
                if not line or line.startswith("#") or "=" not in line:
                    continue
                k, v = line.split("=", 1)
                k, v = k.strip(), v.strip()
                if k == "WEB_USER":
                    user = v
                elif k == "WEB_PASS":
                    pw = v
    else:
        # 兜底仅在 web.env 缺失时生效，提醒用户尽快配置
        print("[auth] 警告: data/web.env 不存在，正在使用默认账号 admin/admin123，请尽快配置随机密码。")
    return user, pw


def check_auth(request) -> bool:
    token = request.cookies.get("session")
    return bool(token) and token in SESSIONS


def login(username: str, password: str) -> bool:
    user, pw = load_creds()
    # 常量时间比较，避免计时侧信道
    return secrets.compare_digest(username, user) and secrets.compare_digest(password, pw)


def create_session() -> str:
    token = secrets.token_hex(16)
    SESSIONS.add(token)
    return token


def destroy_session(token: str) -> None:
    SESSIONS.discard(token)
